...
Router.get('/account', authMiddleware, async (req, res) => {
  try {
    ...
    if (result) {
      ...
      res.status(400).send({
        get_error: 'Данные аккаунта не существуют.'
      });
    }
  } catch (error) {
    res.status(400).send({
      get_error: 'Ошибка при получении данных аккаунта.'
    });
  }
});
...
Router.post('/account', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.send({
      add_error: 'Ошибка при создании нового аккаунта.'
    });
  }
});
...
Router.patch('/account', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.send({
      update_error: 'Ошибка при обновлении аккаунта.'
    });
  }
});
