...
Router.post('/signup', async (req, res) => {
  try {
    ...
    if (count > 0) {
      return res.status(400).send({
        signup_error: 'Пользователь с таким email уже существует.'
      });
    }
    ...
    res.status(400).send({
      signup_error: 'Ошибка во время регистрации.'
    });
  }
});
...
Router.post('/signin', async (req, res) => {
  try {
    ...
    if (!user) {
      res.status(400).send({
        sigin_error: 'Электронная почта/пароль не соответствует.'
      });
    }
    ...
    if (!result.rows[0]) {
      return res.status(400).send({
        signin_error: 'Ошибка во время входа.'
      });
    }
    ...
    res.status(400).send({
      signin_error: 'Электронная почта/пароль не соответствует.'
    });
  }
});
...
Router.post('/logout', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(400).send({
      logout_error: 'Ошибка во время выхода.'
    });
  }
});
